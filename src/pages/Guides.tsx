...
import { organicGuides } from "@/data/guideArticles";
    ...
    <EditorialShell>
      ...
      <main>
        ...
        <section className="site-container py-16 sm:py-24"><div className="grid gap-6 lg:grid-cols-2">
          ...
          <article className="topic-card flex flex-col justify-between p-7 sm:p-9"><div><p className="eyebrow">Organic cluster</p><h2 className="mt-4 text-2xl font-bold">Ten focused field notes for real search questions.</h2><p className="mt-4 text-sm leading-7 text-foreground/65">System requirements, gameplay, factions, fleets, multiplayer and honest fit checks—written for players, not copied from an affiliate offer.</p></div><AffiliateCta ctaId="guides_preview_start_playing" ctaLocation="guides_card" pageType="guides" className="mt-8 w-fit px-6 py-3 text-[11px]">Start Playing</AffiliateCta></article>
        </div><div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{organicGuides.map((article, index) => <Link key={article.slug} to={`/guides/${article.slug}`} className="topic-card group p-6 transition hover:-translate-y-1 hover:border-primary"><p className="eyebrow">{article.eyebrow}</p><h2 className="mt-4 text-xl font-bold leading-tight group-hover:text-primary">{article.title}</h2><p className="mt-3 text-sm leading-7 text-foreground/65">{article.description}</p><span className="mt-5 block text-xs font-semibold uppercase tracking-[0.18em] text-primary">Read field note {String(index + 2).padStart(2, "0")} →</span></Link>)}</div></section>
      </main>
    </EditorialShell>
  );
}
